...
move_logic:
    ...
    srl $t0, $t0, 19 #get top bits to delay
    #how long to loop
...
move_valid:
	addi $t0, $0, 1
	sw $t0, 0x201c($0) # assume we will move
	sw $0, 0x2008($0) #store big data, reset
    	...
    	sw $0, 0x200c($0) #reset the vector
    	lw $t2, 0($t1) 	#check what is at our new location
    	bne $t2, $0, move_finish #if the new location is not a 0, skip location update
    	sw $t1, 0x2000($0) # store our new location
    	j move_finish
move_finish:
...
ekey:
	addi	$t1, $0, 0x0024 # t1 is set to E key constant
	bne	$t0, $t1, qkey #skip if not our key
	######QIS FOR CHANGING POKEMON######
	lw	$t2, 0x2004($0)
	addi	$t2, $t2, 1
	sw	$t2, 0x2004($0)
	j	end_loop	
qkey:
	addi	$t1, $0, 0x0015 # t1 is set to Q key constant
	bne	$t0, $t1, right #skip if not our key
	######QIS FOR CHANGING POKEMON######
	lw	$t2, 0x2004($0)
	addi	$t2, $t2, -1
	sw	$t2, 0x2004($0)
